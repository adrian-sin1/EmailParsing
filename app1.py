...
st.markdown("Upload a raw exported email CSV file. This app will parse and group replies by thread (subject). Select threads to export them as CSV, Excel, or Notepad.")
...
for row in reader:
    ...
    # Exchange internal address handling
...
df["__order"] = range(len(df))  # For sorting
...
with st.form("upload_form"):
    st.markdown("**Enter your Council Connect credentials and automation preferences:**")
    cc_username = st.text_input("Council Connect Username", type="default")
    cc_password = st.text_input("Council Connect Password", type="password")
    cc_auto_click = st.checkbox("Auto-click 'Create Casework' after each form", value=True)
    upload_trigger = st.form_submit_button("🚀 Upload Selected Threads")

if upload_trigger:
    if result_df is None or result_df.empty:
        st.error("Please select and export at least one thread before uploading.")
    elif not cc_username or not cc_password:
        ...
        with st.spinner("Launching browser and submitting entries..."):
            ...
            DRIVER_PATH = os.path.join(os.getcwd(), "msedgedriver.exe")  # or wherever your driver is

            try:
                upload_to_council_connect(result_df, cc_username, cc_password, cc_auto_click, DRIVER_PATH)
